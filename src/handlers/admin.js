...
module.exports = (bot) => {
  ...
  bot.action("admin_battlepass", async (ctx) => {
    ...
    await ctx.reply(
      ...
      "`skip_all_30` — всем пропустить 30 уровней (максимум)\n" +
      "`skip_123456789_30` — пользователю 30 уровней",
      { parse_mode: "Markdown" }
    );
  });
  ...
  bot.action("admin_broadcast", async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.reply("📢 *Рассылка*\n\nИспользуй команду:\n`/broadcast ID сообщение`\nИли `/broadcast all сообщение`", { parse_mode: "Markdown" });
  });
  ...
  bot.hears("📦 Выдать паки", async (ctx) => {
    const userId = ctx.from.id;
    if (!isAdmin(userId)) return;
    await ctx.reply("📦 *Выдать паки*\n\nФормат: `ID тип количество`\n\nТипы: basic, premium, legendary\n\nПример: `123456789 basic 3`", { parse_mode: "Markdown" });
  });

  bot.hears("🎁 Сезонный пак", async (ctx) => {
    const userId = ctx.from.id;
    if (!isAdmin(userId)) return;
    await ctx.reply("🎁 *Сезонный пак*\n\nОтправь ID пользователя:\n`123456789`\nИли `all`\n\nМожно указать количество: `ID 3`", { parse_mode: "Markdown" });
  });

  bot.hears("🎖️ Пропуск уровней", async (ctx) => {
    const userId = ctx.from.id;
    if (!isAdmin(userId)) return;
    await ctx.reply("🎖️ *Пропуск уровней*\n\n`ID 5` или `all 10` или `skip_all_30`", { parse_mode: "Markdown" });
  });

  bot.hears("📢 Рассылка", async (ctx) => {
    const userId = ctx.from.id;
    if (!isAdmin(userId)) return;
    await ctx.reply("📢 *Рассылка*\n\nИспользуй команду:\n`/broadcast ID сообщение`", { parse_mode: "Markdown" });
  });
  ...
  bot.hears("🔙 Главное меню", async (ctx) => {
    ...
    await ctx.reply("🏒 *Bullet Kings*\n\nГлавное меню:", { parse_mode: "Markdown", ...mainMenu });
  });
  ...
  bot.on("text", async (ctx) => {
    ...
    // 🔥 ПРОПУСК УРОВНЕЙ: skip_all_30 или skip_ID_30
    if (text.startsWith("skip_")) {
      const skipParts = text.split("_");
      if (skipParts.length === 3) {
        const target = skipParts[1];
        const levels = parseInt(skipParts[2]);
        if (!isNaN(levels) && levels > 0 && levels <= 30) {
          const users = getUsers();
          if (target === "all") {
    ...
    if (parts.length === 3 && !isNaN(parts[0]) && ["basic", "premium", "legendary"].includes(parts[1]) && !isNaN(parts[2])) {
      ...
      if (count < 1 || count > 10) { await ctx.reply("❌ Количество должно быть от 1 до 10!"); return; }
      const targets = target === "all" ? Object.keys(users) : [target];
      let successCount = 0;
      for (const id of targets) {
        if (!users[id]) continue;
      ...
      saveUsers(users);
      const packNames = { basic: "Базовый", premium: "Премиум", legendary: "Легендарный" };
      await ctx.reply("✅ *" + packNames[packType] + " пак выдан " + successCount + " пользователям!*\n\n📦 Каждому выдано по " + count + " пак(ов)\n📨 Каждому отправлено уведомление с кнопкой 'Открыть'", { parse_mode: "Markdown" });
      await ctx.reply("✅ Готово!", { reply_markup: { remove_keyboard: true } });
      return;
    }
    ...
    if (parts.length >= 1 && (parts[0] === "all" || (!isNaN(parts[0]) && parts[0] !== "all"))) {
      ...
      const target = parts[0];
      let count = 1;
      if (parts.length >= 2 && !isNaN(parts[1])) { count = parseInt(parts[1]); if (count < 1) count = 1; if (count > 10) count = 10; }
      if (target === "all") {
        const ids = Object.keys(users);
        let successCount = 0;
        for (const id of ids) {
    ...
    if (parts.length >= 2 && !isNaN(parts[0]) && parts[0] !== "all") {
      ...
      if (!users[target]) { await ctx.reply("❌ Пользователь " + target + " не найден!"); return; }
      const { PLAYERS } = require('../data/players');
